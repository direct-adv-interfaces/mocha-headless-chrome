...
function startCoverage(page) {return function() {
	return Promise.all([
...
function stopCoverage(page) {return function(save) {
	return Promise.all([
		page.coverage.stopJSCoverage(),
		page.coverage.stopCSSCoverage(),
	]).then(saveCoverage(save));
};}

function saveCoverage({jsPath, cssPath} = {}) {
	return async function([js, css]) {
		const save = (path, data) => fs.writeFile(path, JSON.stringify(data));
		if( jsPath) await save( jsPath,  js);
		if(cssPath) await save(cssPath, css);
		return {js, css};
	};
}
    ...
    return new Promise(resolve => {
        ...
        const result = puppeteer
            ...
            .then(browser => browser.pages()
                ...
                .then(async page => {
                    await page.exposeFunction('puppeteerStartCoverage', startCoverage(page));
                    await page.exposeFunction('puppeteerStopCoverage', stopCoverage(page));
